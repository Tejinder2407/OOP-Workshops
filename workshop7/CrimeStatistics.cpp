...
namespace sdds {
    ...
    void CrimeStatistics::cleanList() {
        std::vector <Crime> crimes_WithoutNone(crimeCollection.size());
        std::transform(crimeCollection.begin(), crimeCollection.end(), crimes_WithoutNone.begin(), [](Crime crimeDetail) {
        ...
        crimeCollection = crimes_WithoutNone;
    };
    ...
    std::list<Crime> CrimeStatistics::getListForProvince(std::string provinceName) const {
        std::vector <Crime> crimes_InProvinceVector(crimeCollection.size());
        auto it = std::copy_if(crimeCollection.begin(), crimeCollection.end(), crimes_InProvinceVector.begin(), [=](const Crime& crimeDetail) {
            return crimeDetail.province == provinceName;
            });

        unsigned int size = it - crimes_InProvinceVector.begin();
        std::list <Crime> crimesInProvinceList(size);
        std::copy(crimes_InProvinceVector.begin(), it, crimesInProvinceList.begin());
